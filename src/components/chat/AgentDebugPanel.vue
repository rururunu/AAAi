<template>
  <aside class="agent-debug-panel" :class="{ embedded }" :aria-label="tr(settingStore.language, 'runtime.title')">
    <header class="debug-header">
      ...
      <div class="debug-actions">
        ...
        <button class="icon-btn" type="button" :title="tr(settingStore.language, 'runtime.clear')" @click="clear">
          <Trash2 :size="13" />
        </button>
        <button v-if="!embedded" class="icon-btn" type="button" :title="tr(settingStore.language, 'runtime.close')" @click="emit('close')">
          <X :size="14" />
        </button>
      </div>
    </header>
    ...
    <div v-if="!selectedRun" class="debug-empty">
      {{ tr(settingStore.language, "runtime.waiting") }}
    </div>

    <div v-else-if="tab === 'events'" ref="eventListRef" class="debug-scroll event-list peek-scrollbar">
    ...
    <div v-else-if="tab === 'tools'" class="debug-scroll tool-list peek-scrollbar">
      <article v-for="tool in selectedRun.tools" :key="tool.callId" class="tool-card">
        <header>
          ...
          <span :class="['tool-status', tool.success === false ? 'failed' : tool.result ? 'done' : 'running']">
        ...
        <details open>
          ...
          <div class="payload-block">
            <button type="button" class="copy-btn" :title="tr(settingStore.language, 'runtime.copyArguments')" @click="copyValue(`args-${tool.callId}`, stringify(tool.arguments))">
        ...
        <details v-if="tool.result">
          ...
          <div class="payload-block">
            <button type="button" class="copy-btn" :title="tr(settingStore.language, 'runtime.copyResult')" @click="copyValue(`result-${tool.callId}`, tool.result)">
      ...
      <p v-if="!selectedRun.tools.length" class="inline-empty">{{ tr(settingStore.language, "runtime.noTools") }}</p>
    </div>
      ...
      >
        ...
        <div v-if="isSubagentExpanded(agent)" class="subagent-body">
          <div class="subagent-description-row">
            <p class="subagent-description">{{ agent.description || tr(settingStore.language, "runtime.task") }}</p>
            <button type="button" class="copy-btn" :title="tr(settingStore.language, 'runtime.copyTask')" @click="copyValue(`subagent-task-${agent.id}`, agent.description || tr(settingStore.language, 'runtime.task'))">
        ...
        <ol v-if="agent.progress.length" class="subagent-progress">
          <li v-for="(progress, index) in agent.progress" :key="`${progress.timestampMs}-${index}`">
        ...
        <div v-if="agent.tools.length" class="subagent-tools">
          ...
          <details v-if="agent.summary">
            ...
            <div class="payload-block">
              <button type="button" class="copy-btn" :title="tr(settingStore.language, 'runtime.copyFinalResult')" @click="copyValue(`subagent-result-${agent.id}`, agent.summary)">
...
function eventDetail(event: AgentEvent) {
  ...
  if (event.type === "toolResult") return `${event.data.tool} · ${event.data.success ? "success" : "failed"}`;
...
<style scoped>
.agent-debug-panel { position: absolute; z-index: 12; inset: 33px 0 0 auto; width: min(620px, 52%); min-width: 380px; display: flex; flex-direction: column; overflow: hidden; color: var(--peek-text); background: color-mix(in srgb, var(--peek-list-bg) 97%, transparent); border-left: 1px solid var(--peek-border); box-shadow: -12px 0 26px rgba(0, 0, 0, .18); }
.agent-debug-panel.embedded { position: relative; z-index: auto; inset: auto; flex: 1; width: 100%; min-width: 0; height: 100%; border: 0; background: transparent; box-shadow: none; }
.debug-header, .run-bar, .debug-actions, .debug-heading, .tool-card header { display: flex; align-items: center; }
.debug-header { min-height: 38px; justify-content: space-between; padding: 0 10px 0 12px; border-bottom: 1px solid var(--peek-border); }
.debug-heading, .tool-card header { gap: 7px; }
.debug-heading strong { font-size: 12px; }
.run-count { min-width: 18px; height: 18px; display: inline-grid; place-items: center; padding: 0 5px; border-radius: 4px; background: color-mix(in srgb, var(--peek-text) 7%, transparent); color: var(--peek-muted); font: 9px/1 var(--font-mono); }
.debug-actions { gap: 3px; }
.icon-btn { width: 26px; height: 26px; display: grid; place-items: center; border: 0; border-radius: 4px; color: var(--peek-muted); background: transparent; cursor: pointer; }
.icon-btn:hover { color: var(--peek-text); background: var(--peek-hover); }
.icon-btn:disabled { opacity: .35; cursor: default; }
.run-bar { gap: 8px; padding: 8px 10px; border-bottom: 1px solid var(--peek-border); }
.run-bar select { min-width: 0; flex: 1; height: 28px; color: var(--peek-text); background: var(--peek-input-bg); border: 1px solid var(--peek-border); border-radius: 4px; padding: 0 7px; }
.state-pill, .tool-status { font-size: 10px; text-transform: uppercase; color: var(--peek-muted); }
.state-pill[data-state="completed"], .tool-status.done { color: #58b887; }
.state-pill[data-state="failed"], .tool-status.failed { color: #e06c75; }
.debug-tabs { display: flex; height: 32px; padding: 3px 8px 0; gap: 2px; border-bottom: 1px solid var(--peek-border); }
.debug-tabs button { border: 0; border-bottom: 2px solid transparent; color: var(--peek-muted); background: transparent; padding: 0 9px; font-size: 11px; cursor: pointer; }
.debug-tabs button.active { color: var(--peek-text); border-bottom-color: var(--peek-accent); }
.debug-tabs span { margin-left: 4px; opacity: .7; }
.debug-scroll { min-height: 0; flex: 1; overflow: auto; padding: 10px; }
.event-list { display: flex; flex-direction: column; gap: 5px; }
.event-row { display: grid; grid-template-columns: 25px minmax(0, 1fr); gap: 6px; padding: 7px; border-radius: 5px; }
.event-row:hover { background: color-mix(in srgb, var(--peek-text) 4%, transparent); }
.event-sequence { font: 10px ui-monospace, monospace; color: var(--peek-muted); }
.event-title { display: flex; justify-content: space-between; gap: 8px; font-size: 11px; }
.event-actions { flex: none; display: flex; align-items: center; gap: 4px; }
.event-title time { color: var(--peek-muted); font: 10px ui-monospace, monospace; }
pre { margin: 5px 0 0; white-space: pre-wrap; overflow-wrap: anywhere; color: var(--peek-muted); font: 10px/1.5 ui-monospace, monospace; }
.tool-list { display: flex; flex-direction: column; gap: 8px; }
.tool-card { padding: 9px; border: 1px solid var(--peek-border); border-radius: 6px; background: color-mix(in srgb, var(--peek-input-bg) 78%, transparent); }
.tool-card header strong { flex: 1; font-size: 11px; }
.tool-card p, summary { color: var(--peek-muted); font-size: 10px; }
details { margin-top: 7px; }
summary { cursor: pointer; }
.payload-block { position: relative; min-width: 0; margin-top: 5px; padding: 8px 30px 8px 9px; border-radius: 5px; background: color-mix(in srgb, var(--peek-text) 4%, transparent); }
.payload-block pre { margin: 0; }
.copy-btn { flex: none; width: 23px; height: 23px; display: inline-grid; place-items: center; padding: 0; border: 0; border-radius: 4px; background: transparent; color: var(--peek-faint); cursor: pointer; }
.copy-btn:hover { color: var(--peek-text); background: color-mix(in srgb, var(--peek-text) 7%, transparent); }
.payload-block > .copy-btn { position: absolute; top: 4px; right: 4px; }
...
.subagent-header, .subagent-meta, .subagent-tool-row { display: flex; align-items: center; gap: 7px; }
.subagent-header { width: 100%; min-height: 34px; padding: 0 9px; border: 0; color: var(--peek-text); background: transparent; text-align: left; cursor: pointer; }
.subagent-header strong { flex: 1; font-size: 11px; }
...
.subagent-description { margin: 7px 0 5px; color: var(--peek-text); font-size: 11px; line-height: 1.45; white-space: pre-wrap; overflow-wrap: anywhere; }
.subagent-description-row { display: flex; align-items: flex-start; gap: 6px; }
.subagent-description-row .subagent-description { flex: 1; min-width: 0; }
.subagent-description-row .copy-btn { margin-top: 4px; }
.subagent-meta { color: var(--peek-muted); font: 9px ui-monospace, monospace; }
.subagent-meta time { margin-left: auto; }
.subagent-progress { margin: 8px 0 0; padding: 0; list-style: none; }
.subagent-progress li { display: grid; grid-template-columns: 72px minmax(0, 1fr); gap: 6px; padding: 4px 0; border-top: 1px solid color-mix(in srgb, var(--peek-border) 55%, transparent); font-size: 10px; }
.subagent-progress span { color: var(--peek-muted); }
.subagent-progress strong { min-width: 0; overflow-wrap: anywhere; font-weight: 500; }
.subagent-tools { margin-top: 7px; border-top: 1px solid var(--peek-border); }
.subagent-tool-row { min-height: 27px; font-size: 10px; }
.subagent-tool-row span { flex: 1; }
